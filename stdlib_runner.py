#!/usr/bin/env python3
"""
This script uses only Python standard library modules for initial execution.
It is designed to work in environments where requests module is not available
and pip installation permissions are restricted.
"""
...
def main():
    # Create working directory in a location we should have write access to
    work_dir = Path(tempfile.gettempdir()) / "freeroot_env"
    work_dir.mkdir(exist_ok=True)
    os.chdir(work_dir)
    
    print(f"{WHITE}{'='*70}{RESET_COLOR}")
    print(f"{CYAN}                       FreeRoot Installer{RESET_COLOR}")
    print(f"{CYAN}               Standard Library Edition for Jupyter{RESET_COLOR}")
    print(f"{WHITE}{'='*70}{RESET_COLOR}")
